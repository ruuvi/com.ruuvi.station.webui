...
export default function SensorTypePicker(props) {
    ...
    let opts = Object.keys(allUnits)
        .map((x) => (allUnits[x].graphable ? x : null))
        .filter((x) => x !== null);
    if (props.sensors) {
        let sensorTypes = [];
    ...
    if (props.dashboard) {
        ...
        if (humidityIndex !== -1) {
            ...
            if (humidityUnits) {
                ...
                opts.splice(humidityIndex, 1, ...humidityUnits.map((u) => `humidity_${u.cloudStoreKey}`));
            }
        }
        opts = [null, ...opts];
    }
    ...
    const getLabelForOption = (value) => {
        ...
        if (props.dashboard && sensorType === "humidity" && unit) {
            ...
            if (sensorType === "humidity" && unit && unit.startsWith("2") && unit.length > 1) {
                ...
                let tempUnitLabel =
                    allUnits["temperature"]?.units?.find((x) => x.cloudStoreKey === tempUnit)?.translationKey ||
                    tempUnit;
        ...
        <Menu.Root positioning={{ strategy: "fixed", placement: "bottom-end" }}>
            <Menu.Trigger asChild>
                <Button {...style} borderRadius="4px">
                    <Box pl={1} flex="1 0 auto" minW={0} className={props.altStyle ? "" : "ddlItemAlt"}>
            ...
            <Portal>
                <Menu.Positioner zIndex={100}>
                    <Menu.Content maxHeight="600px" overflowY="auto">
                        {opts.map((op, i) => {
